...
been exacerbated by Anaconda’s new compilers, which caused conda-forge to
become incompatible with the official Anaconda packages, leading to broken
software when users mix the two channels. One of the major advantages of
using conda-forge over individual user channels is its consistent build
